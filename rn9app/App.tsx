...
import {mobileNetCheckpoint, resNet50Checkpoint} from './src/checkpoints';
import {assertValidOutputStride, assertValidResolution, getInputTensorDimensions, getValidInputResolutionDimensions, padAndResizeTo, scaleAndFlipPoses, toTensorBuffers3D, validateInputResolution} from './src/util';
import {BaseModel} from './src/base_model';
import {decodeSinglePose} from './src/single_pose/decode_single_pose';
import { Tensor3D } from '@tensorflow/tfjs';
import { MobileNet } from './src/mobilenet';
import { ResNet } from './src/resnet';
import * as posenetCustom from './src/posenet_model';

import {InputResolution} from './src/types';
...
const TensorCamera = cameraWithTensors(Camera);
 ...
 class App extends React.Component<{}, AppState> {
  ...
  async loadHigherHRNetModel(){
    ...
    const inputResolution: InputResolution = { width: inputTensorWidth, height: inputTensorHeight };
    const url = resNet50Checkpoint(outputStride, multiplier, quantBytes);
    const graphModel = await tfconv.loadGraphModel(url);
    // console.log(graphModel)
    const mobilenet = new MobileNet(graphModel, outputStride);
    const validInputResolution = getValidInputResolutionDimensions(
      inputResolution, outputStride);

    return new posenetCustom.PoseNet(mobilenet, validInputResolution);
  }
  ...
  async handleImageTensorReady(
    ...
    const loop = async () => {
      ...
      if (this.state.higherHRNetModel != null) {
        const imageTensor = images.next().value;
        const flipHorizontal = Platform.OS === 'ios' ? false : true;
        const hrnet_output = await this.state.higherHRNetModel.estimateSinglePose(
          imageTensor, { flipHorizontal });
