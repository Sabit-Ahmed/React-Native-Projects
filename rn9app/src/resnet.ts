...
const imageNetMean = [0.5, 0.5, 0.5];
...
export class ResNet extends BaseModel {
  ...
  nameOutputResults(results: tf.Tensor3D[]) {
    const [displacementFwd, displacementBwd, offsets, heatmap, extra] = results;
    return {offsets, heatmap, displacementFwd, displacementBwd, extra};
  }
}
