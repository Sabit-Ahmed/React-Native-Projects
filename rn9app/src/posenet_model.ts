...
export class PoseNet {
  ...
  async estimateSinglePose(
    ...
    const outputStride = this.baseModel.outputStride;
    const inputResolution: [number, number] = [256, 256];
    ...
    // const {heatmapScores, offsets, displacementFwd, displacementBwd} =
    //     this.baseModel.predict(resized);
    const {heatmapScores, offsets, displacementFwd, displacementBwd, extra} =
        this.baseModel.predict(resized);

    // console.log(heatmapScores)

    const pose = await decodeSinglePose(displacementBwd, displacementFwd, outputStride);
    // console.log(pose)
    const poses = [pose];
